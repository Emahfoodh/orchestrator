...
# Configuration
APP_DIR="../srcs/api-gateway"
VENV_DIR="$APP_DIR/venv"
REQUIREMENTS_FILE="$APP_DIR/requirements.txt"
RUN_FILE="$APP_DIR/run.py"

log_info "Starting API Gateway setup and launch..."

# Check if app directory exists
if [ ! -d "$APP_DIR" ]; then
    log_error "API Gateway directory not found: $APP_DIR"
    exit 1
fi
...
log_success "Starting API Gateway..."
log_info "API Gateway will be available at: http://localhost:5000"
log_info "This service routes requests to:"
log_info "  - Inventory API: http://localhost:8080"
log_info "  - Billing API: http://localhost:8081"
